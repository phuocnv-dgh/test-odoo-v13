...
###############################################################################################################
    ...
    def _compute_session_token(self, sid):
        ...
        print('####### session_fields: ', session_fields)
        print('####### self.id from res_security: ', self.id)
        # query = """SELECT %s, (SELECT value FROM ir_config_parameter WHERE key='database.secret')
        #                         FROM res_users
        #                         WHERE id=%%s""" % (session_fields), (self.id,)
        # print('######## query from res_users.py: ', query)
        ...
        print('######### self.env.cr from res_users: ', self.env.cr)
        print('######### data_fields from res_users: ', data_fields)
        # generate hmac key
        key = (u'%s' % (data_fields,)).encode('utf-8')
        print('########### key from res_user-phuoc: ', key)
        # hmac the session id
        data = sid.encode('utf-8')
        print('########### data from res_user-phuoc: ', data)
        h = hmac.new(key, data, sha256)
        print('########### h from res_user-phuoc: ', h)
        # keep in the cache the token
        res_token = h.hexdigest()
        print('####### res_token: ', res_token)
        return res_token

    # @tools.ormcache('login')
    def getUserByLogin(self, login):
        self.env.cr.execute("""SELECT * FROM res_users WHERE login = %s""", (login,))
