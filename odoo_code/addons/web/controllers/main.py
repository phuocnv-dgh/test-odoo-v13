...
def login_and_redirectCustom(db, login, redirect_url='/web'):
    request.session.authenticateCustom(db, login)
    return set_cookie_and_redirect(redirect_url)
#######################################################################################
...
############################################------SSO------##############################################
    ...
    @http.route('/web/loginCustom', type='http', auth="none")
    def web_login_default(self, redirect=None, **kw):
        tempToken = request.params['token']

        result = requests.get('http://192.168.100.15:3200/odoo/checkSSO/' + tempToken)
        data = result.json()

        if data['success'] == False:
            return http.local_redirect('/web/login')

        login = data['info_user']['email']

        uid = request.session.authenticateCustom("odoo_db", login)
        if uid == "NotActive":
            return http.local_redirect('/web/login')
...
class Session(http.Controller):
    ...
    @http.route('/web/session/authenticate01', type='json', auth="none")
    def authenticates(self, db, login, base_location=None):
        request.session.authenticateCustom(db, login)
        return request.env['ir.http'].session_info()

###########################################################################################
    @http.route('/web/session/authenticateCustom', type='json', auth="none")
    def authenticate_redirect(self, uid, base_location=None):
        db = "odoo_db"
        result = requests.get('http://192.168.100.15:3200/odoo/checkSSO/' + uid)
        data = result.json()
        login = data['info_user']['email']
        return login_and_redirectCustom(db=db, login=login)
###########################################################################################
