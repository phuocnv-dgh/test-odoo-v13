...
##############################################################################################################
    def authenticateCustom(self, db, login=None, password=None, uid=None):
        ...
        if uid is None:
            ...
            #### TRA VE ID CUA LOGIN ###
            data = security.getUid(request.env, login)
            array_data = []
            for item in data:
                array_data.append(item)
            if array_data[1] == True:
                uid = array_data[0]
            else:
                return uid == "NotActive"
        else:
            security.check(db, uid, password)
        self.rotate = True
        self.db = db
